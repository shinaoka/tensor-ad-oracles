...
def _replay_structural_success_case(
    ...
    *,
    ...
    op_kwargs = dict(record.get("op_kwargs", {}))
    input_names = tuple(inputs.keys())
    observable = structural.apply(torch, record["op"], inputs, op_kwargs)
    output_names = tuple(observable.keys())
    ...
    plus_output = structural.apply(torch, record["op"], plus_inputs, op_kwargs)
    minus_output = structural.apply(torch, record["op"], minus_inputs, op_kwargs)
    fd_jvp = {
        name: (plus_output[name] - minus_output[name]) / (2.0 * fd_step)
        for name in output_names
    }
...
def _replay_success_case(
    ...
    *,
    ...
    stored_fd_jvp = decode_tensor_map(record["probes"][0]["fd_ref"]["jvp"])
    if getattr(spec, "inventory_kind", "linalg") == "local_structural":
        _replay_structural_success_case(
            torch,
            record=record,
            inputs=inputs,
            direction=direction,
