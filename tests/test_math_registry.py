...
class MathRegistryTests(unittest.TestCase):
    ...
    def test_repo_lu_note_defines_scalarized_sum_loss_rule(self) -> None:
        ...
        self.assertIn("all-ones matrices with the", text)
        self.assertIn("same shapes as $L$ and $U$", text)
        self.assertIn("full-shape cotangents", text)
        self.assertIn("structurally zero entries of either factor", text)
    ...
    def test_repo_qr_note_defines_scalarized_sum_loss_rule(self) -> None:
        ...
        self.assertIn("same shapes as $Q$ and $R$", text)
        self.assertIn("d\\phi_{\\mathrm{qr}}[dA]", text)
        self.assertIn("\\left\\langle \\mathbf{1}_Q, dQ", text)
        self.assertIn("\\left\\langle \\mathbf{1}_R, dR", text)
        self.assertIn("not masked to the triangular support", text)
        self.assertIn("cancel against the leading-block", text)
