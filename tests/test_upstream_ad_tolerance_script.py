...
class CheckUpstreamAdTolerancesScriptTests(unittest.TestCase):
    ...
    def test_audit_skips_local_inventory_families_without_upstream_opinfo(self) -> None:
        spec_index = {
            ("where", "identity"): SimpleNamespace(
                upstream_name="where",
                upstream_variant_name="",
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            (root / "where").mkdir()
            path = root / "where" / "identity.jsonl"
            path.write_text(
                json.dumps(
            ...
            with (
                ...
                patch.object(
                    ...
                    side_effect=AssertionError("local families should not resolve upstream tolerances"),
                ),
            ):
                audits = check_upstream_ad_tolerances.audit_against_upstream_ad_tolerances(root)
